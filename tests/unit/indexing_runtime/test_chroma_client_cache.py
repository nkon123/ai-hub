...
`indexing_runtime.chroma_client_cache` module docstring and
...
tests/unit/search_runtime/test_chroma_client_cache.py by design — the
production module it tests (`indexing_runtime.chroma_client_cache`) is a
deliberate, documented duplicate of `search_runtime.chroma_client_cache`
(see that module's docstring for why: CLAUDE.md 원칙 2 forbids M07/M08
importing each other's internals, and there is no existing shared
vector-store adapter package to route through instead)."""
...
import pytest
from indexing_runtime import chroma_client_cache
from indexing_runtime.chroma_client_cache import cache_size, clear_cache, get_chroma_client
...
def test_same_path_returns_the_same_client_object_across_many_calls(tmp_path: Path) -> None:
    """The core fix: N calls against one path (N indexing jobs against the
    same Knowledge asset, e.g. repeated re-indexing) must reuse one client,
    not construct (and leak) a new one every time."""
...
def test_cache_never_exceeds_configured_max_size(tmp_path: Path, monkeypatch) -> None:
    ...
    is itself a slow leak (many Knowledge indexes indexed over a service's
...
def test_eviction_actually_closes_the_client_not_just_forgets_it(
    ...
    # Indexing a second, distinct Knowledge asset with max size 1 must
    # evict the first.
...
def test_lru_order_evicts_least_recently_used_not_least_recently_inserted(
    ...
    that only tracked insertion order would evict a hot, frequently
    re-indexed Knowledge asset instead of a cold one."""
