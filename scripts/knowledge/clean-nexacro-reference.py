...
**CHM 을 가지고 있다면 이 스크립트 대신 `chm-html-to-markdown.py` 를 쓴다.**
이 스크립트는 **이미 변환된** Markdown 에서 잔재를 걷어내는 것이라, 그 변환기가
이미 뭉개 버린 정보(Property Type 체크 상태, 지원 환경의 미지원 항목, Structure
다이어그램, 표 구조)는 되살릴 수 없다. HTML 에서 직접 만들면 전부 살아난다 —
실측으로 섹션별 대조했을 때 한글이 줄어든 섹션 0개, 늘어난 섹션 8,372개였다.
이 스크립트는 원본 CHM 없이 기존 md 만 있을 때를 위해 남겨 둔다.
