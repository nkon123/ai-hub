...
// `truncateToolResultText`로 길이를 잘라 저장한다(240자 — 카드 한두 줄에
// 들어가는 "요약" 이상을 저장하지 않는다).
//
// **`schedule-history-store.ts`의 상한(20,000자)과 일부러 다르다.** 한때
// 두 값이 같았고 주석도 "같은 상한"이라고 적고 있었지만, 스케줄 이력의
// 전제가 다르다는 것이 실사용으로 드러나 그쪽만 올렸다(2026-08-19):
// 대화는 사람이 화면에서 바로 읽고 지나가므로 요약이면 충분하지만,
// 스케줄은 사람이 안 보는 사이 돌았으므로 **그 기록이 곧 결과물**이라
// 240자면 기능 자체가 무의미해진다. 두 파일은 별도 모듈이라 함수를
// 공유하지 않고 각자 상수를 갖는다 — 한쪽을 바꿀 때 다른 쪽을 따라
// 바꾸지 마라. 다른 것이 의도다. Tool을 전혀 쓰지 않은
