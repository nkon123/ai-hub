...
- **출처 칩은 "무엇이 검색됐는지"를 말해야 한다.** 예전에는 파일명만 보여줘서 같은 문서에서 여러 조각이 걸리면 칩이 전부 같은 글자였다(2026-09-18 실사용 제보). 지금은 `chatTypes.describeCitationChip`(순수, 테스트 고정)이 **주제(섹션)를 앞세우고** 문서명·페이지·발췌 한 줄을 붙인다. 없는 값은 지어내지 않는다(섹션/제목이 모두 없으면 "제목 없음"). 전체 발췌는 클릭해 여는 모달이 그대로 보여준다 — 칩에 전부 넣지 않는다.
