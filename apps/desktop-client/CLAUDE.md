...
- **대화 화면 좌측 목록은 "대화 | 봇" 탭이다**(2026-09-18 사용자 요청). 스케줄봇(스케줄 실행 이력, 읽기 전용)은 **봇 탭에만** 있고 대화 목록에 섞지 않는다. 탭을 바꾸면 오른쪽도 따라간다 — 봇 탭을 열면 스케줄봇이 바로 열리고, 대화 탭으로 돌아가면 보던 대화가 그대로다(`handleSidebarTabChange`). "새 대화" 버튼은 대화 탭의 **맨 아래**, 스크롤 영역 밖에 있다. 탭은 `ui.tsx` 의 `Tabs`(`compact`, `disabled`)를 쓴다 — 답변 생성 중에는 전환하지 않는다.
