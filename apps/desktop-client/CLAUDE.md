...
- **"+" 메뉴 행의 오른쪽 값(detail)에는 짧은 것만 넣는다.** `shrink-0` 만 준 칸에 긴 문장이 들어가면 왼쪽 라벨 칸이 0 에 가깝게 눌려 글자가 **세로로 한 글자씩** 쏟아진다(2026-09-18 실사용: "필요하면 Tool 자동 선택"이 세로로 길게 겹쳐 보였다 — 후보 Tool **이름 목록**이 그 자리에 통째로 들어갔었다). 지금은 `max-w-[45%] truncate` + `title` 로 잘라 보여주고, 긴 문구는 그 행의 **설명(description)** 자리로 내린다.
