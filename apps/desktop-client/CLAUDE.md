...
- 대화 화면의 "프롬프트" 버튼(`src/screens/PromptPickerPanel.tsx`)은 허브에서 받은 Prompt 자산을 골라 **질문 입력창에 문구를 넣기만 한다** — agent-runtime의 system prompt/템플릿으로 적용하지 않는다(그 경로는 D-034 해석 경로 2/4의 Agent+Prompt 짝이며 이 버튼과 무관하다). 문구를 "적용"이라고 부르지 않는 이유가 이것이다: 보내는 것은 언제나 사용자가 입력창에서 눈으로 확인한 텍스트다. 목록 정렬·텍스트 조립·최근 사용 기록은 `src/screens/promptPickerTypes.ts`(순수, 테스트가 고정)에 있고, 본문 파일 읽기는 `getPromptTemplate` IPC → `electron/asset-management.ts::readPromptTemplate` → `electron/prompt-template.ts`(경로 담기 검사)다. 최근 사용 기록은 D10 설정이 아니라 렌더러 `localStorage`에 둔다(사용자가 고르는 값이 아니라 사용 흔적이며, 읽기/쓰기 실패는 "최근 목록 없음"으로 degrade한다).
- 대화 화면 입력창의 켤 수 있는 것들은 **"+" 메뉴 하나**로 접혀 있다(`src/screens/ComposerMenu.tsx` — 지식 / 프롬프트 / 도구(Tool 자동·MCP·로컬) / 검색(허브)). 모델 선택과 전송 버튼은 접지 않는다(매 턴 보는 값). **켜진 것은 접지 않는다**: `composerMenuTypes.buildActiveChips`가 만든 칩이 입력창에 상시로 남고 눌러서 끌 수 있다 — 접어서 안 보이면 켠 줄 모르는 채로 보내게 되고, 이 줄에는 허브 전송 동의(D-078)처럼 의미가 큰 것도 있다. 항목이 비활성인 **이유**는 화면이 아니라 `composerMenuTypes.itemAvailability`가 정한다(렌더링 테스트가 없는 저장소라 판단을 화면에 두면 고정할 수 없다).
- **출처 칩은 "무엇이 검색됐는지"를 말해야 한다.** 예전에는 파일명만 보여줘서 같은 문서에서 여러 조각이 걸리면 칩이 전부 같은 글자였다(2026-09-18 실사용 제보). 지금은 `chatTypes.describeCitationChip`(순수, 테스트 고정)이 **주제(섹션)를 앞세운다**. 없는 값은 지어내지 않는다(섹션/제목이 모두 없으면 "제목 없음"). **칩은 작은 알약 한 줄이다**(2026-09-18 "출처 칩이 너무 커진다") — 번호·주제·페이지·로컬/허브만 보이고, 문서명과 발췌는 툴팁과 클릭 모달로 간다. 발췌를 다시 칩에 펼치지 않는다(전체 폭 카드가 되어 출처가 답변보다 커 보였다). 머리줄은 `summarizeCitationSources`("출처 5 · 로컬 4 · 허브 1"), 5개 이상이면 `citationChipsToShow` 가 앞 4개만 두고 "+N개"로 접는다. 허브 칩만 색으로 구분한다(D-078 배지는 머리줄과 칩 양쪽에 남는다).
- **"+" 메뉴 행의 오른쪽 값(detail)에는 짧은 것만 넣는다.** `shrink-0` 만 준 칸에 긴 문장이 들어가면 왼쪽 라벨 칸이 0 에 가깝게 눌려 글자가 **세로로 한 글자씩** 쏟아진다(2026-09-18 실사용: "필요하면 Tool 자동 선택"이 세로로 길게 겹쳐 보였다 — 후보 Tool **이름 목록**이 그 자리에 통째로 들어갔었다). 지금은 `max-w-[45%] truncate` + `title` 로 잘라 보여준다. **행 설명(description)은 화면에 보이지 않는다**(2026-09-18 사용자 요청 "상세 설명들은 빼 줘") — `title` 툴팁과 `sr-only` 텍스트로만 남긴다(허브 항목의 "로컬 문서 내용은 전송되지 않습니다"는 D-078 규칙 3 때문에 접근성 트리에 남아야 한다). 비활성 **이유**(`reason`)만 눈에 보인다. 설명을 다시 늘어놓지 않는다.
- **입력창 위에 떠야 하는 것은 `absolute` 로 두지 않는다**(2026-09-18 실사용: "+ 버튼 누르면 리스트가 깨진다"). 대화 열이 `overflow-hidden` 이라(`ChatScreen.tsx`의 `flex flex-1 flex-col overflow-hidden`) 입력창에서 **위로** 펼쳐지는 absolute 요소는 그 경계에서 잘린다. `ComposerMenu` 는 트리거의 `getBoundingClientRect()` 로 위치를 계산해 `fixed` 로 띄우고(같은 이유로 `ui.tsx` 의 Modal 도 fixed 다), 남은 높이에 맞춰 `maxHeight` + 스크롤을 준다 — 창이 작을 때 잘린 채 두면 아래 항목이 있다는 사실 자체가 안 보인다.
- **MCP 서버 등록은 agent-runtime 재시작에 살아남지 않는다**(서버 레지스트리가 메모리에만 있다). 그래서 Desktop 이 복구한다: `mcp-server-connection.ts::reconcileInstalledMcpServers` 가 로컬 ACTIVE + Active Version 인 것 중 서버 목록에 없는 것만 "다시 확인"과 같은 경로로 재등록한다 — 앱 시작 직후(Main, 재시도 루프)와 대화/설치된 자산 화면을 열 때. 예전에는 이게 없어 "다시 확인"을 눌러야만 대화에 서버가 보였다(2026-09-18 실사용). FAILED 기록은 여기서 재시도하지 않는다 — 설정이 안 바뀌면 또 거절된다.
- **대화 화면 좌측 목록은 "대화 | 봇" 탭이다**(2026-09-18 사용자 요청). 스케줄봇(스케줄 실행 이력, 읽기 전용)은 **봇 탭에만** 있고 대화 목록에 섞지 않는다. 탭을 바꾸면 오른쪽도 따라간다 — 봇 탭을 열면 스케줄봇이 바로 열리고, 대화 탭으로 돌아가면 보던 대화가 그대로다(`handleSidebarTabChange`). "새 대화" 버튼은 대화 탭의 **맨 아래**, 스크롤 영역 밖에 있다. 봇 탭 맨 아래에는 **비활성** "봇 생성(향후 지원)" 버튼이 자리만 잡고 있다(동작 없음 — 누를 수 있는 척하지 않도록 `disabled` + `title`). 화면 맨 위의 "채팅" 제목 줄은 없앴다 — 연결 상태 점과 보조 아이콘은 **오른쪽 대화 영역 위**에 있어 왼쪽 탭이 화면 맨 위부터 시작한다. 탭은 `ui.tsx` 의 `Tabs`(`compact`, `disabled`)를 쓴다 — 답변 생성 중에는 전환하지 않는다.
- **대화방·스케줄 삭제는 사유를 묻지 않는다**(2026-09-18 사용자 결정, D-102) — `ConfirmDialog`(확인만)이고 스토어·IPC 에도 `reason` 인자가 없다. 이 PC 의 내 대화/스케줄 정리이지 공유 자산의 승인·반려·폐기가 아니다. 스케줄 **비활성화**는 예정된 실행을 멈추는 결정이라 사유(`ReasonConfirmDialog`)를 그대로 받는다. 자산 버전 전환·제거 같은 공유 설치 변경도 그대로다.
- 세 패널(프롬프트/MCP/로컬 Tool)은 `showTrigger={false}` + `externalOpen`으로 메뉴가 열고, **입력창에 항상 마운트돼 있다** — 메뉴가 닫히면서 패널까지 언마운트되면 방금 연 대화상자가 함께 사라진다.
- 대화 화면의 "MCP 도구" 버튼(`src/screens/McpToolPickerPanel.tsx`)은 이번 대화에서 **AI가 고를 수 있는 Tool 범위**를 정한다(프롬프트 버튼과 같은 자리지만 성격이 다르다 — 저쪽은 입력창 텍스트, 이쪽은 런타임 후보). 서버명을 누르면 그 서버의 Tool 전체, 펼쳐서 개별 Tool만 고를 수도 있고, 첫 줄의 "자동 선택"은 연결된 전체에서 AI가 고르게 한다. **고른다고 반드시 실행되는 것이 아니다** — 무엇을 부를지는 agent-runtime의 TOOL_ROUTE가 질문을 보고 정하고 승인 절차는 그대로다. 화면 문구를 "적용/실행"으로 바꾸지 않는다.
  범위 계산은 `src/screens/mcpToolScopeTypes.ts`(순수, 테스트가 고정)에 있고, 전송할 때 `scopeToRunParams`가 세 경우를 구분한다: **생략=후보 전체**(자동), 목록=그만큼으로 좁힘, `toolRoute:false`=Tool 안 씀. **빈 배열을 만들지 않는다** — 서버에서 "후보 없음"이라는 다른 뜻이다.
- **지식·도구를 아무것도 켜지 않은 턴은 agent-runtime 을 거치지 않는다**(`ChatScreen.tsx` 의 `ollamaOnly` → `bridge.chatWithOllama`, Main process 가 Ollama 를 직접 부른다). 그래서 그 턴은 서버 쪽 `stage.timing` 로그에 **아무것도 남지 않는다** — "hi 만 보냈는데 로그가 안 올라온다"(2026-09-18 실사용)는 정상이다. 이 경로의 소요시간은 Main process 로그(`chat` 카테고리, 첫 글자/전체/모델/답변 길이)에 남는다.
- 그 경로는 2026-09-18부터 **스트리밍**이다(`chat:ollamaDelta` 이벤트 → `onOllamaChatDelta`). 예전에는 `stream: false` 라 답이 다 만들어질 때까지 화면이 비어 있었고, 그 때문에 아무것도 안 붙인 기본 대화가 지식 검색을 켠 경로보다 느리게 느껴졌다. `chatWithOllama` 에 `onDelta` 를 주지 않으면 예전처럼 한 번에 받는다(Agent 초안 생성처럼 흘려보낼 화면이 없는 호출).
...
- 새 IPC 채널을 추가했다면 `types.ts`/`main.ts`/`preload.ts`/`browserPreviewBridge.ts` 네 곳을 모두 갱신했는가(`src/bridge.ts`의 `Record<keyof DesktopBridge, true>` 맵과 `src/bridge.test.ts`의 가짜 bridge도 함께 — 둘 다 `pnpm typecheck`가 즉시 잡는다)(마지막 하나를 빠뜨려도 `pnpm typecheck`가 잡아준다 — `BrowserSettingsBridge`가 `DesktopBridge` 전체 타입이기 때문).
