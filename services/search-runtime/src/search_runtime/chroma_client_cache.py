...
resolved Chroma directory path — one client reused across every search
query against that path, instead of a brand-new client per call.

Why this exists (docs/implementation-spec/open-decisions.md D-067):
`hybrid_search()` used to construct a fresh `chromadb.PersistentClient` on
every single query and never close it. Chroma's Rust bindings back each
*distinct* persist directory with its own `System`
...
search-runtime process serves many DIFFERENT Knowledge indexes over its
...
Duplicated in `indexing_runtime.chroma_client_cache` rather than shared via
a `packages/*` import: CLAUDE.md 원칙 2 forbids M07 (indexing-runtime)
...
(already true in `hybrid_search`; this cache only ever holds the
`PersistentClient` itself, never a `Collection` object), immediately
reflects writes committed through a completely different client/process
pointed at the same path (tested: a second client deleted+recreated+
repopulated a collection, and the original long-lived client's next
`get_collection()` call saw the new row count, not the old one). This
matches what Chroma was already doing before this fix — the underlying
`System` for a given path was already shared/reused process-wide — so
whatever staleness behavior existed previously is unchanged by caching the
thin client wrapper on top of it.
"""
...
from search_runtime.settings import CHROMA_CLIENT_CACHE_MAX_SIZE
